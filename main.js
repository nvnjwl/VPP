...
const GRID_COLS = 20;
const GRID_ROWS = 6;
const AUTO_DETECT_INTERVAL = 1000;
const MAX_AUTO_QUADS = 3;
...
function extractQuadFromContours(srcMat) {
  ...
  const quads = candidates.slice(0, MAX_AUTO_QUADS).map(({ approx }) => {
    const points = [];
    for (let i = 0; i < 4; i += 1) {
  ...
  candidates.slice(MAX_AUTO_QUADS).forEach(({ approx }) => approx.delete());
  return quads;
}

function isQuadDuplicate(points) {
  const centroid = quadCentroid(points);
  return quads.some((existing) => {
    const existingCentroid = quadCentroid(existing);
    const distance = Math.hypot(centroid.x - existingCentroid.x, centroid.y - existingCentroid.y);
    return distance < 12;
  });
}
...
async function autoDetectQuad() {
  ...
  const quadPointsList = extractQuadFromContours(src);
  src.delete();

  if (!quadPointsList.length) {
    setAutoStatus("No quad found");
    return;
  }

  let addedCount = 0;
  quadPointsList.forEach((quadPoints) => {
    const mappedPoints = quadPoints.map(mapVideoPointToCanvas);
    const normalized = normalizeQuad(mappedPoints);
    if (!isQuadDuplicate(normalized)) {
      quads.push(normalized);
      addedCount += 1;
    }
  });
  setAutoStatus(addedCount ? `Added ${addedCount} quad(s)` : "No new quads");
}
...
function drawQuadOverlays(bestIndex) {
  ...
  quads.forEach((quad, index) => {
    const color =
      index === bestIndex ? "rgba(59, 130, 246, 0.8)" : "rgba(59, 130, 246, 0.35)";
